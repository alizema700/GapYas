...
ax.set_xlabel(r"$\nu$ (Exponent der Kanten $|x|$, $|y|$)"); ax.set_ylabel(r"$\mu$ (Exponent der Kante $|x-y|$)")
present = sorted({r["phase"] for r in rows}, key=phases.index)
for p in present:
    ax.plot([], [], "s", color=cols[phases.index(p)], label={"hex": "hexagonal", "square": "Quadrat", "rect": "Rechteck", "rhombic": "rhombisch", "centred-rect": "zentr. Rechteck", "oblique": "schief"}[p])
...
ax.plot(x2, y2, "s-", color="#d1242f", label="2D: hexagonal → Quadrat")
ax.set_yscale("log"); ax.set_xlabel(r"Exponent $\nu$"); ax.set_ylabel(r"kritisches $\lambda_c=c_3/c_2$")
