...
s = nu / 2; R = 40
log = open("prove_nu%g.log" % nu, "w")
def say(*a):
    msg = " ".join(str(x) for x in a); print(msg, flush=True); log.write(msg + "\n"); log.flush()

# ---- T(i) and local lemma -------------------------------------------------------------------
c3, e3, BR = T_jet(0.0, 1.0, nu, R=R, K=3)
Ti_up = c3[0, 0] + e3[0, 0]; Ti_lo = c3[0, 0] - e3[0, 0]
say("nu=%g  T(i) in [%.12f, %.12f]" % (nu, Ti_lo, Ti_up))
...
while queue:
    ...
    if x1 <= rl and 1 - rl <= y0 and y1 <= 1 + rl: continue           # inside the local-lemma box
...
say("DONE nu=%g: all %d boxes certified in %.0fs -> square lattice is the unique global minimiser" % (nu, n_boxes, time.time() - t0))
json.dump(dict(nu=nu, T_i=[Ti_lo, Ti_up], r_loc=rloc, Y0=Y0, boxes=n_boxes), open("prove_nu%g.json" % nu, "w"), indent=1)
