...
ax.plot(nus, np.array(dh) * 100, color=C["sq"], label=r"$T_\square/T_{\mathrm{hex}}-1$")
tr = v2b["track"]; kk = sorted(tr, key=float)
ax.plot([float(k) for k in kk if float(k) <= 12], [(tr[k]["rect_T"] / tr[k]["hex_T"] - 1) * 100 for k in kk if float(k) <= 12],
...
fig.savefig("figures/fig_2d_energy_difference.pdf"); plt.close(fig)
...
ax.legend(frameon=False, fontsize=8); fig.savefig("figures/fig_2d_stability.pdf"); plt.close(fig)
...
fig.savefig("figures/fig_2d_landscape.pdf"); plt.close(fig)
...
fig.savefig("figures/fig_2d_arc_barrier.pdf"); plt.close(fig)
...
fig.savefig("figures/fig_2d_rect_ratio.pdf"); plt.close(fig)

# Fig 6: 3D FCC/BCC/SC relative to the best found lattice
rows = {}
...
if rows:
    ...
    fig, ax = plt.subplots(figsize=(5.2, 2.8))
    for name, col in [("FCC", C["fcc"]), ("BCC", C["bcc"]), ("SC", C["sc"])]:
        ax.plot([float(k) for k in ks], [(rows[k]["reference"][name] / rows[k]["best_found"] - 1) * 100 for k in ks], "o-", ms=3, color=col, label=name)
    ax.axhline(0, color="k", lw=0.6)
    ax.set_xlabel(r"Exponent $\nu$"); ax.set_ylabel(r"$T/T_{\mathrm{best}}-1$ in %"); ax.set_yscale("symlog", linthresh=0.1)
    ax.legend(frameon=False, fontsize=8); fig.savefig("figures/fig_3d_lattices.pdf"); plt.close(fig)
