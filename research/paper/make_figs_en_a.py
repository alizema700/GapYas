...
ax.plot(nus, np.array(dh) * 100, color=C["sq"], label=r"square: $T_{\mathrm{sq}}/T_{\mathrm{hex}}-1$")
...
ax.plot([float(k) for k in kk if float(k) <= 12], [(min(tr[k]["rect_T"], tr[k]["square_T"]) / tr[k]["hex_T"] - 1) * 100 for k in kk if float(k) <= 12],
        "o", ms=3, color=C["rect"], label=r"best rectangle$/T_{\mathrm{hex}}-1$")
...
ax.set_xlabel(r"exponent $\nu$"); ax.set_ylabel("relative energy difference (%)"); ax.set_ylim(-60, 8)
...
ax.plot([float(k) for k in ks], [st[k]["square_eigs"][0] for k in ks], "s-", ms=3, color=C["sq"], label="square")
...
ax.set_xlabel(r"angle $\arg\tau$ (deg; 60 = hex, 90 = square)"); ax.set_ylabel(r"$T_{\nu^*}(\tau)$")
...
ax.set_xscale("log"); ax.set_xlabel(r"exponent $\nu$"); ax.set_ylabel("aspect ratio $b/a$ of the optimum")
