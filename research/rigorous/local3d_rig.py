...
# c: exact rational value from bcc_local_exact.py (c^2 = 27/1400), rigorous lower bound
c = 0.138873014965
for tri in act:
    f, g, _ = f_grad_hess(tri, q0)
    assert f.a <= mpf(3) / 2 <= f.b
print("c (rigorous lower bound, exact c^2 = 27/1400):", c)
...
def M_of(r):
    ...
    for tri in act:
        ...
        fro = iv.sqrt(sum(iv.mpf(max(abs(h.a), abs(h.b))) ** 2 for row in H for h in row))   # rigorous upper bound
        M = max(M, fro.b)
    return M
...
for r in [0.007]:
    M = M_of(r)
    ok = iv.sqrt(5) * r < 2 * iv.mpf(c) / iv.mpf(M)   # interval comparison
    ok = bool(ok) if not hasattr(ok, "__iter__") else ok
    res.append(dict(r=r, M=str(M), ok=bool(ok)))
    print("r=%.4f  M<=%s  lemma holds on box (interval check sqrt5*r < 2c/M): %s" % (r, M, ok))
json.dump(dict(c=c, n_active=len(act), results=res), open("local3d_rig.json", "w"), indent=1)
