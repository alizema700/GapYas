"""Rigorous computer-assisted proof for the rectangular regime nu > nu_2: the unique global minimiser of T_nu
lies in a small box around the float candidate tau0 = i*y0 and hence (symmetry x -> -x) on the imaginary axis.
Variant of prove_min_rig.py: the local lemma uses strong convexity plus a small gradient at the centre.  All quantities are rigorous enclosures (jets_rig.py);
...
nu = float(sys.argv[1]); POINT = "rect"; Y0p = float(sys.argv[2])   # float candidate on the imaginary axis
s = arb(nu) / 2; E3 = 3 * s
X0 = 0.0
R = 40
logf = open("prove_rig_rect_nu%g.log" % nu, "w")
def say(*a):
...
for k in [(1, 0), (1, 1), (3, 0), (1, 2)]:          # odd in A: zero by the symmetry x -> -x (x0 = 0 exactly)
...
def rect_ok(r):
    """strong convexity on |A|,|B| <= r (Hessian lower bound as in hess_ok) and |grad T(center)| < lambda r:
    then T has a unique minimiser in the box (it lies within |grad|/lambda of the centre)"""
    r = arb(r); t = r
    h = (2 * t + 2 * t * t) / (1 - t)
    if h.upper() >= 1: return False
    hp = (2 + 4 * t - 2 * t * t) / (1 - t) ** 2; hpp = 8 / (1 - t) ** 3
    ...
    H22 = 2 * c[(0, 2)] - 2 * mag(c[(1, 2)]) * r - 6 * mag(c[(0, 3)]) * r - rho2
    H12 = mag(c[(1, 1)]) + 2 * mag(c[(2, 1)]) * r + 2 * mag(c[(1, 2)]) * r + rho2
    if H11.lower() <= 0 or H22.lower() <= 0: return False
    lam = ((H11 + H22) - ((H11 - H22) ** 2 + 4 * H12 * H12).sqrt()) / 2
    if lam.lower() <= 0: return False
    grad = (arb(mag(c[(1, 0)])) ** 2 + arb(mag(c[(0, 1)])) ** 2).sqrt()
    return (grad - lam * r).upper() < 0
test = rect_ok
rloc = 0.0
for r in np.geomspace(1e-6, 0.1, 300):
    if test(r): rloc = float(r)
    elif rloc > 0: break
say("  local lemma (strong convexity + small gradient, |c01| = %.3e) holds for max(|A|,|B|) <= r_loc = %.5f" % (mag(c[(0, 1)]), rloc))
assert rloc > 0
...
say("DONE nu=%g: all %d boxes certified in %.0fs (rigorous arithmetic) -> the unique global minimiser is rectangular, tau = i y with |y - %.10f| <= r_loc y0" % (nu, n, time.time() - t0, Y0p))
json.dump(dict(point=POINT, nu=nu, T0=[str(T0.lower()), str(T0.upper())], r_loc=rloc, Y0=Yc, boxes=n), open("prove_rig_rect_nu%g.json" % nu, "w"), indent=1)
